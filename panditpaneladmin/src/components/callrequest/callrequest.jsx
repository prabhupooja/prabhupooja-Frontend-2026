...
function Callrequest() {
      ...
      if (response.status === 200) {
        ...
        if (status === 'accepted') {
          if (requestType === 'chat') {
            ...
            initiateCall(astroId, userId, requestType, user_mobile, pandit_mobile, requestId);
          }
        }
      } else {
  ...
  const initiateCall = async (astroId, userId, type, user_mobile, pandit_mobile, requestId) => {
    try {
      ...
      if (response.data.success) {
        Swal.fire('Success', 'Call initiated successfully', 'success');
  ...
  if (loading) {
    return (
      <>
        <div style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "5vh", marginTop: "50px" }}>
