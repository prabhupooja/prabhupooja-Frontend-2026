...
function Callrequest() {
  ...
  const { type } = location.state || { type: "voice" };
    ...
    const initAndFetch = async () => {
      try {
        ...
        const fetchRequests = async (targetId) => {
          try {
            const response = await api.get(`/request/showforpandit/${targetId || pId}/${type || "voice"}`);
            setRequests(response?.data?.data || []);
          } catch (err) {
            console.warn("Failed to load call requests:", err?.message || err);
          } finally {
            setLoading(false);
          }
        };
        ...
        console.error("Error in Callrequest init:", err);
        setLoading(false);
      }
    };
      ...
      if (response.status === 200 || response.data?.success) {
        ...
        if (status === "accepted") {
          Swal.fire({
            ...
            text: userMobile ? `Connecting call with Devotee (${userMobile})...` : "Connecting to consultation...",
  ...
  if (loading) {
    return (
      <div className="userlist-container request-loader-box">
        <TailSpin height="50" width="50" color="#ff7a00" />
        <p className="loading_text" style={{ marginTop: "15px", color: "#64748b", fontWeight: "600" }}>Loading voice call requests...</p>
      </div>
    );
  }
    ...
    <div className="userlist-container">
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "20px" }}>
        <h1 style={{ fontSize: "1.5rem", fontWeight: "800", color: "#0f172a", margin: 0, display: "flex", alignItems: "center", gap: "8px" }}>
          <FaPhoneVolume style={{ color: "#10b981" }} /> Devotee Voice Call Requests
        </h1>
        <span style={{ background: "#ecfdf5", color: "#047857", padding: "6px 14px", borderRadius: "20px", fontSize: "0.82rem", fontWeight: "700" }}>
        ...
        <table className="user-table" style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ background: "#f8fafc", textAlign: "left" }}>
              ...
              <th style={{ padding: "14px 16px", color: "#475569" }}>Contact</th>
            ...
            {requests.length > 0 ? (
                ...
                <tr key={user.request_id || user.id || index} style={{ borderBottom: "1px solid #f1f5f9" }}>
                  ...
                  <td style={{ padding: "14px 16px" }}>
                    {user.user_mobile || user.mobile ? (
                      <a href={`tel:${user.user_mobile || user.mobile}`} style={{ color: "#10b981", fontWeight: "700", textDecoration: "none" }}>
                        📞 {user.user_mobile || user.mobile}
                      </a>
                    ) : (
                      "—"
              ...
              <tr>
                <td colSpan="6" style={{ textAlign: "center", padding: "40px", color: "#94a3b8", fontSize: "0.95rem" }}>
                  No pending voice call requests at the moment.
                </td>
              </tr>
            )}
          </tbody>
