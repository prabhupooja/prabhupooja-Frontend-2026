...
const useProblemPoojaStore = create((set) => ({
        ...
        try {
            ...
            if (response.data?.success) {
                ...
                if (Array.isArray(bookingData) && bookingData.length > 0) {
                    set({ bookingDate: bookingData[0]?.bookingdate || null });
                } else if (bookingData && typeof bookingData === "object" && bookingData.bookingdate) {
                    set({ bookingDate: bookingData.bookingdate });
                } else {
    ...
    getBookings: async (userId) => {
        try {
            ...
            if (response?.data?.success) {
                set({ bookings: response.data.data || response.data || [] });
                return response;
            }
